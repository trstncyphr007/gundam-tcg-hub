...
# The web app is its own Next project: it does not read the root .env, it reads .env.local
# beside itself. So it is handed the two values it needs and none of the secrets — a process
# that never holds a database password cannot leak one, and that is the whole reason this is a
# separate file rather than a symlink to the root .env.
#
# Rewritten every run, including the run that finds an existing .env, so a checkout that
# predates this file still gets one.
write_web_env() {
  s3_origin="$(grep -m1 '^PHOTO_STORAGE_ORIGIN=' .env 2>/dev/null | cut -d= -f2- || true)"
  # A .env generated before this variable existed still names the bucket, just not under this
  # name. Fall back to it rather than quietly writing a default that disagrees with it.
  if [ -z "$s3_origin" ]; then
    s3_origin="$(grep -m1 '^S3_ENDPOINT=' .env 2>/dev/null | cut -d= -f2- || true)"
  fi
  umask 077
  cat > apps/web/.env.local <<EOF
# Generated by scripts/init-env.sh. Local development only - never commit.
API_INTERNAL_URL=http://127.0.0.1:4000
# The origin the *browser* reaches the photo bucket on. It is named in the site's CSP:
# connect-src for the presigned PUT, img-src for the processed photo. Without it the browser
# refuses both, in the console, where nothing on the server ever sees it.
PHOTO_STORAGE_ORIGIN=${s3_origin:-http://127.0.0.1:8000}
EOF
  echo "Wrote apps/web/.env.local (mode 600)."
}
...
CLAMAV_PORT=3310
# The origin the *browser* reaches the bucket on, which is what the site's CSP has to name:
# connect-src for the presigned PUT, img-src for the processed photo. Separate from
# S3_ENDPOINT because in production they need not be the same host - the API may reach the
# bucket over an internal name no browser can resolve.
PHOTO_STORAGE_ORIGIN=http://127.0.0.1:8000
