...
| TB3 | App → Postgres                   | Yes, with per-service roles and row-level security                                            |
...
| T16 | API abuse / DoS                                   | Per-minute rate limit (in-memory); **per-day key quota in Postgres, so a restart no longer refills it** (ADR-042); 1 MB body limit; 15 s request timeout. Refusals are **recorded** and alerted on (ADR-037/038)                            | A shared store for the minute window once there are two instances; Cloudflare at deploy                               |
| —   | Info disclosure via errors                        | 5xx responses return `internal_error` only; tested. A **sweep over every authenticated write route** asserts none of them answers 5xx at all, which the nightly fuzzer cannot check — it has no session and sees only the eight public GETs | —                                                                                                                     |
...
| Container escape / lateral movement         | Non-root, read-only, all capabilities dropped (the minimum re-added for Postgres), no-new-privileges, per-service CPU/memory/PID limits                          |
| Reaching the database from outside          | Postgres sits on an `internal: true` network with **no published ports** — Docker cannot publish from it at all (proven locally)                                 |
| Services nobody uses                        | Valkey ran in both stacks for weeks with nothing connected to it, and `up -d --wait` meant it could fail a deploy. Removed (ADR-042)                             |
