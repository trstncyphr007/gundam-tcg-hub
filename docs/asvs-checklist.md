...
| 16.4 | Alerting on security events | **Partial** | A failed nightly job or backup reports through one `gth-alert@` unit to the ops webhook, and `preflight.sh` fails when the timers are off or no webhook is configured (ADR-036). The rest of SR-X.22 has no data source yet — nothing records a failed login, and the rate limiter keeps no ban — so there is nothing yet to alert on. Application-side, and next                                                              |
